...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
      isScrolled ? "py-3 bg-gradient-to-r from-[#2C7865] to-[#004445] shadow-xl" 
                : "py-2 bg-gradient-to-r from-[#2C7865] to-[#004445]"
    }`}>
      <div className="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <div className="flex-shrink-0">
            <Link to="/" className="text-2xl font-bold text-white transition duration-300 hover:text-[#FFD700]">
              Helping Hands
            </Link>
          </div>
          ...
          <div className="hidden md:flex md:items-center md:space-x-4">
            {navConfig[userRole].map((item, index) => (
              <Link key={index} to={item.path} className="px-3 py-2 text-sm font-medium text-white hover:text-[#FFD700]">

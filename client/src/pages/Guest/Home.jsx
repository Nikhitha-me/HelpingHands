    ...
    <div>
      <div className="relative h-[740px] flex items-center justify-center bg-cover bg-center overflow-hidden">
        ...
        <div className="relative z-10 text-center text-white">
          ...
          <motion.button whileHover={{ scale: 1.1 }} className="bg-[#90D26D] text-white px-8 py-3 rounded-full hover:bg-[#004445]">
            Donate Now
          </motion.button>
        </div>
      </div>
  ...
  <div className="modal-overlay fixed inset-0 z-[9999] flex items-center justify-center bg-black bg-opacity-50">
    <div className="p-6 text-center bg-white rounded-lg">
      ...
      <button
        ...
        className="bg-[#90D26D] text-white px-6 py-2 rounded-lg hover:bg-[#004445]"
      >
        Go to Login
      </button>
    </div>
...
const ArrowButton = ({ direction, onClick }) => (
  <button
    onClick={onClick}
    className={`absolute ${direction === "left" ? "left-[-50px]" : "right-[-50px]"} top-1/2 transform -translate-y-1/2 bg-[#90D26D] text-white p-3 rounded-full hover:bg-[#004445]`}
    style={{ zIndex: 10 }}
  >
